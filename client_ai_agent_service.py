...
# Client-specific AI configuration for agent service
class ClientAIAgentConfig:
    def __init__(self):
        ...
        try:
            ...
            print(f"Warning: Client AI Agent Gemini client initialization failed: {e}")
            self.client = None
    ...
    def generate_content(self, prompt, max_tokens=200, temperature=0.7):
        """Generate content using Google Gemini for client agent"""
        if self.client is None:
            raise Exception("Client AI Agent Gemini client not initialized")
        ...
        try:
            ...
            logging.error(f"Client AI Agent Gemini API error: {str(e)}")
            raise Exception(f"Client AI Agent generation failed: {str(e)}")
    
    def is_configured(self):
        """Check if client AI agent is properly configured"""
        return self.client is not None and self.api_key is not None

client_ai_agent_config = ClientAIAgentConfig()

class ClientAIAgentService:
    """Client AI Agent service for automated company tasks"""
    
    def __init__(self):
        self.ai_config = client_ai_agent_config
    ...
    def _handle_inventory_management(self, task: AIAgentTask) -> Dict[str, Any]:
        ...
        # Define language at the beginning to avoid unbound variable issues
        language = task.parameters.get('language', 'en') if task.parameters else 'en'
...
client_ai_agent_service = ClientAIAgentService()